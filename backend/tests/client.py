...
class ResultClientMixin(BaseClient):
    ...
    def list_results(self, project_id, build_id, params=None):
        url = self.url("/projects", project_id, "/builds", build_id, "/results")
        return requests.get(url, params=params)

    @log_response
    def create_result(self, project_id, build_id, data, params=None, headers=None):
        url = self.url("/projects", project_id, "/builds", build_id, "/results")
        return requests.post(url, data=json.dumps(data), params=params, headers=headers)

    @log_response
    def create_result_junit_xml(
        self, project_id, build_id, data, params=None, headers=None
    ):
        url = self.url("/projects", project_id, "/builds", build_id, "/results")
    ...
    def get_result(self, project_id, build_id, result_id, params=None):
        url = self.url(
            "/projects", project_id, "/builds", build_id, "/results", result_id
        )
        return requests.get(url, params=params)
...
class LastCountByStatusClientMixin(BaseClient):
    @log_response
    def list_last_x_by_status_results(self, project_id, status, count, params=None):
        url = self.url("/projects", project_id, "/status", status, "/count", count)
        return requests.get(url, params=params)


class LastCountByTestNameClientMixin(BaseClient):
    @log_response
    def list_last_x_by_test_name_results(
        self, project_id, test_name, count, params=None
    ):
        url = self.url(
            "/projects", project_id, "/test_name", test_name, "/count", count
        )
        return requests.get(url, params=params)
