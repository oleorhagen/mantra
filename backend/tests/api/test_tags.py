...
class BuildTagsTest(BaseBuildsTest):
    ...
    def test_filter_build_with_tags(self):
        ...
        self._create_build(
            project_id=self.project_id, name="no-match", tags={"a": "no-match"},
        )

        build_2 = self._create_build(
            project_id=self.project_id, name="matches-bbb", tags={"b": "bbb"},
        ).json()
        ...
        resp = self.client.list_builds(self.project_id, params={"a": "aaa", "b": "bbb"})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.json()), 1)
        self.assertEqual(resp.json()[0], self.build)
...
class ResultTagsTest(BaseResultTest):
    ...
    def test_get_result_with_tags(self):
        resp = self.client.get_result(self.project_id, self.build_id, self.result["id"])
        self.assertEqual(resp.status_code, 200)
