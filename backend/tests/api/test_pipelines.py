...
class TestPipelines(BasePipelinesTest):
    ...
    def test_list_piplines(self):
        resp = self.client.list_pipelines(self.project_id)
        self.assertEqual(resp.status_code, 200)
        self.assertGreater(len(resp.json()), 0)

    def test_get_build(self):
        resp = self.client.get_pipeline(self.pipeline_id, self.build["id"])
        self.assertEqual(resp.status_code, 200)
...
class TestBuildsStringTruncation(BasePipelinesTest):
